...
const DeveloperSettingsApp: React.FC = () => {
  ...
  const { confirm, alert, prompt, passwordPrompt } = useDialog();
  ...
  const handlePerformanceModeChange = (mode: 'high' | 'balanced' | 'low') => {
    ...
    info("Performance Mode Updated", `Performance mode changed to ${mode}`);
  };
  ...
  const handleAnimationToggle = () => {
    ...
    info("Animation Setting", `Animations ${!enableAnimations ? 'enabled' : 'disabled'}`);
  };
  ...
  const handleTransparencyToggle = () => {
    ...
    info("Transparency Setting", `Transparency ${!reduceTransparency ? 'reduced' : 'normalized'}`);
  };
  ...
  const handleExperimentalFeaturesToggle = () => {
    if (!experimentalFeatures) {
      ...
      confirm("Experimental Features", "Enabling experimental features may cause instability. Are you sure you want to proceed?")
        .then((result) => {
          if (result) {
            setExperimentalFeatures(true);
            debugLog('Experimental features toggled', { enabled: true });
            warning("Experimental Features", "Experimental features have been enabled. Restart the application for full effect.");
          }
        });
    } else {
      ...
      success("Experimental Features", "Experimental features have been disabled.");
    }
  };
  ...
  const applyCustomTheme = () => {
    ...
    success("Custom Theme Applied", "Your custom theme has been applied successfully.");
  };
  ...
  const testErrorApiCall = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        warning("Unexpected Success", "Expected error but got success response");
      } else {
        throw new Error(data.message || `API call failed with status ${response.status}`);
      }
    } catch (error) {
  ...
  const testDialog = () => {
    ...
    confirm("Developer Test Dialog", "This is a test dialog from the Developer Settings panel. You can use dialogs to show important information or get user input.")
      .then((result) => {
        debugLog('Test dialog closed', { result });
        info("Dialog Test", `Dialog result: ${result ? 'Confirmed' : 'Cancelled'}`);
      });
  };
  ...
  const testNotification = (type: 'info' | 'success' | 'warning' | 'error' = 'info') => {
    ...
    const notificationMethods = { info, success, warning, error };
    notificationMethods[type](title, message);
  };

  // Test error notification
  const testErrorNotification = () => {
    debugLog('Test error notification shown');
    error("Error Simulation", "This is a simulated error notification for testing purposes.");
  };

  // Test warning notification
  const testWarningNotification = () => {
    debugLog('Test warning notification shown');
    warning("Warning Simulation", "This is a simulated warning notification for testing purposes.");
  };

  // Test success notification
  const testSuccessNotification = () => {
    debugLog('Test success notification shown');
    success("Success Simulation", "This is a simulated success notification for testing purposes.");
  };
  ...
  const updateSystemStats = () => {
    ...
    info("System Stats", "System stats are monitored in real-time. Values update automatically.");
  };

  // Reset all settings
  const resetSettings = () => {
    debugLog('Reset settings requested');
    confirm("Reset Developer Settings", "Are you sure you want to reset all developer settings to their defaults? This cannot be undone.")
      .then((result) => {
        if (result) {
          ...
          success("Settings Reset", "Developer settings have been reset to defaults.");
        }
      });
  };
  ...
  const clearCache = () => {
    ...
    confirm("Clear Cache", "Are you sure you want to clear the application cache? This may temporarily affect performance.")
      .then((result) => {
        if (result) {
          // In a real app, this would clear the cache
          debugLog('Cache cleared');
          success("Cache Cleared", "Application cache has been cleared successfully.");
        }
      });
  };
